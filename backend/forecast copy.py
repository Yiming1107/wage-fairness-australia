...
- Strict exact matching (no fuzzy matching)
"""
...
def fuzzy_match(target, choices, cutoff=None):
    """
    Exact match: ignore case, punctuation, and spaces only.
    No fuzzy matching - must be exact after normalization.
    cutoff parameter is kept for compatibility but ignored.
    """
    ...
    if len(target) > MAX_STRING_LENGTH:
        logger.warning(f"Match target too long: {len(target)}")
    ...
    for choice in choices:
        if normalize(choice) == target_norm:
            return choice  # 返回原始格式
